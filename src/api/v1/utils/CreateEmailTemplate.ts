class CreateEmailTemplate {
  ...
  static emailTemplateBottom: string = `
    </div>
  </body>
  </html>`;

  static createEmailTemplate(reason: string, token: string): string {
    const htmlContent = `
    <p>
    ...
    return this.emailTemplateTop + htmlContent + this.emailTemplateBottom;
  }
}
